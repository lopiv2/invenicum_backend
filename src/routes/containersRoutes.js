...
router.post('/containers', verifyToken, async (req, res) => {
    try {
        ...
        const result = await containerService.createContainer(req.user.id, containerData);
...
router.put('/containers/:id', verifyToken, async (req, res) => {
    try {
...
router.put('/containers/:containerId/items/:id', verifyToken, async (req, res) => {
    try {
        // Verificar que el contenedor pertenece al usuario
        const container = await containerService.getContainerById(parseInt(req.params.containerId), req.user.id);
        if (!container) {
...
router.delete('/containers/:containerId/items/:id', verifyToken, async (req, res) => {
    try {
        // Verificar que el contenedor pertenece al usuario
        const container = await containerService.getContainerById(parseInt(req.params.containerId), req.user.id);
        if (!container) {
...
router.patch('/containers/:containerId/items/:id/options', verifyToken, async (req, res) => {
    try {
        // Verificar que el contenedor pertenece al usuario
        const container = await containerService.getContainerById(parseInt(req.params.containerId), req.user.id);
        if (!container) {
