...
// Import tus tools de base de data and crypto
...
async function runFullIntegrationTest() {
  ...
  try {
    ...
    // --- PASO 2: decrypted MANUAL (Igual que en tu AIService) ---
    console.log("🔓 Paso 2: Desencriptando datos manualmente...");
    let apiKeyToUse;
    let modelToUse = "gemini-3-flash-preview";

    try {
      // Desencriptamos the string que viene de config.data
    ...
    if (ogImage) {
      // CASO 1: Es a route relativa (ej: "/img/foto.jpg")
      if (ogImage.startsWith("/")) {
        ogImage = `${baseUrl.origin}${ogImage}`;
      }
      // CASO 2: No tiene protocolo (ej: "www.apple.com/foto.jpg" o "apple.com/foto.jpg")
      else if (!ogImage.startsWith("http")) {
        // if empieza por 'www', le ponemos the https://
        // if es a route relativa without barra (ej: "media/foto.jpg"), the unimos al origen
        ogImage =
    ...
    // 1. Limpieza de bloques Markdown (the error que te dio)
